...
/*   Updated: 2025/05/26 17:20:50 by dimachad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  *(unsigned int *)(fl->img.addr + px_position) = color;
  ...
  if (quadratic) {
    if ((px.i_y + 1) < RES_HIGHT) {
      px_pos_nxt_line =
          ((px.i_y + 1) * fl->img.line_len) + (px.r_x * (fl->img.bpp / 8.0));
      *(unsigned int *)(fl->img.addr + px_pos_nxt_line) = color;
    }
    if (px.r_x + 1 < RES_WIDTH) {
      px_position =
          (px.i_y * fl->img.line_len) + ((px.r_x + 1) * (fl->img.bpp / 8.0));
      *(unsigned int *)(fl->img.addr + px_position) = color;
      if ((px.i_y + 1) < RES_HIGHT) {
        px_pos_nxt_line = ((px.i_y + 1) * fl->img.line_len) +
                          ((px.r_x + 1) * (fl->img.bpp / 8.0));
        *(unsigned int *)(fl->img.addr + px_pos_nxt_line) = color;
      }
    }
  }
